...
@click.option(
    ...
    required=True,
    type=str,
    help='Model name or path to load with SentenceTransformer.'
)
@click.option(
    '--datasets',
    multiple=True,
...
def main(model, datasets, output):
    ...
    results = evaluation.run(st_model, output_folder=None, eval_splits=["test"])  # output_folder disables MTEB's default file output
